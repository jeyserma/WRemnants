...
class Recoil:
    ...
    def __init__(self, pu_type, args, flavor="mu"):
        ...
        self.axis_res_diff = hist.axis.Regular(20000, -1, 1, name = "res", underflow=False, overflow=False)
    ...
    def recoil_Z(self, df, results, dataset, datasets_to_apply, leps_uncorr, leps_corr):
        ...
        # make obsolete
        self.trgLep_pt = leps_corr[0]
        self.trgLep_eta = leps_corr[1]
        self.trgLep_phi = leps_corr[2]
        self.trgLep_charge = leps_corr[3]
        self.nonTrgLep_pt = leps_corr[4]
        self.nonTrgLep_eta = leps_corr[5]
        self.nonTrgLep_phi = leps_corr[6]
        self.nonTrgLep_charge = leps_corr[7]
    ...
    def recoil_vars_plots_Z(self, rec_corr, nominal_weight="nominal_weight", suffix=""):
        ...
        if not self.df.HasColumn(f"recoil_{rec_corr}_magn"):
            ...
            self.df = self.df.Define(f"met_{rec_corr}_wlike", f"wrem::get_met_wlike({self.nonTrgLep_pt}, {self.nonTrgLep_phi}, met_{rec_corr}_pt, met_{rec_corr}_phi)")
            ...
            self.df = self.df.Define(f"mt_{rec_corr}", f"wrem::get_mt_wlike({self.trgLep_pt}, {self.trgLep_phi}, met_{rec_corr}_wlike)")
            self.df = self.df.Define(f"dphi_{rec_corr}", f"std::abs(wrem::deltaPhi(v_phi, met_{rec_corr}_phi))")
            self.df = self.df.Define(f"dphi_{rec_corr}_wlike", f"std::abs(wrem::deltaPhi({self.trgLep_phi}, met_{rec_corr}_phi_wlike))")
    ...
    def recoil_vars_plots_W(self, rec_corr, nominal_weight="nominal_weight", suffix=""):
        ...
        if not self.df.HasColumn(f"recoil_{rec_corr}_magn"):
            lep_pt = "lep_uncorr_pt" if rec_corr=="uncorr" else "lep_corr_pt"
            lep_phi = "lep_uncorr_phi" if rec_corr=="uncorr" else "lep_corr_phi"
            self.df = self.df.Define(f"recoil_{rec_corr}_magn", f"wrem::compute_recoil_from_met_and_lepton(met_{rec_corr}_pt, met_{rec_corr}_phi, {lep_pt}, {lep_phi})")
            self.df = self.df.Define(f"mt_{rec_corr}", f"wrem::mt_2({lep_pt}, {lep_phi}, met_{rec_corr}_pt, met_{rec_corr}_phi)")
            self.df = self.df.Define(f"dphi_{rec_corr}", f"std::abs(wrem::deltaPhi({lep_phi}, met_{rec_corr}_phi))")
    ...
    def setup_gen_reco_vars_Z(self):
        ...
        if self.dataset.name in self.datasets_to_apply:
            ...
            # proxy pre-FSR
            self.df = self.df.Define("trg_lep_mom4", f"ROOT::Math::PtEtaPhiMVector({self.trgLep_pt}, {self.trgLep_eta}, {self.trgLep_phi}, wrem::muon_mass)")
            self.df = self.df.Define("proxy_gen_prefsr", f"wrem::proxy_gen_v(genl, genlanti, trg_lep_mom4, {self.trgLep_charge})")
            ...
            # proxy post-FSR
            self.df = self.df.Define("proxy_gen_postfsr", f"wrem::proxy_gen_v(lepGen, antilepGen, trg_lep_mom4, {self.trgLep_charge})")
    ...
    def apply_recoil_W(self): 
        ...
        self.add_histo(f"lep_uncorr_pt", ["lep_uncorr_pt"], [self.axis_lep_pt_test], with_fakes=True, coll_passMT=f"passMT_corr_rec")
        self.add_histo(f"lep_corr_pt", ["lep_corr_pt"], [self.axis_lep_pt_test], with_fakes=True, coll_passMT="passMT_corr_rec")
